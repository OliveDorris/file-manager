...
文件管理系统是一个轻量级 Web 文档管理应用，用于在网页端完成登录、文件分类、文档上传、文档下载、文档修改和版本记录。
...
1. 当前 `app.py` 是轻量版单文件实现，路由函数中仍包含数据库操作。
2. 新增功能时应逐步引入 service / repository 分层，但不要一次性大范围重构。
3. 审计日志尚未完善。登录、权限变更、删除数据、审批操作等重要操作后续需要记录用户、时间、IP、操作。
4. 统一错误处理尚未完全抽象。
5. 当前系统主要是单管理员轻量版，尚未实现完整多用户、角色、权限体系。
6. 当前部署是 HTTP + IP + 9000 端口。正式对外使用时建议增加域名、Nginx 反向代理和 HTTPS。
7. 当前 SQLite 和上传文件都在服务器本地磁盘，需要定期备份 `/home/file-manager/data`。
